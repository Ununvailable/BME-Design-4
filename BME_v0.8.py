    ...
    def update_plot(self, i=0):
        if self.FileReaderFlag:
            if self.file_read_index < len(global_file_data):
                self.plot_list[self.index] = self.data_scale * global_file_data[self.file_read_index]
        ...
        if self.ConnectionFlag:
            if self.real_time_index < len(real_time_data):
                self.plot_list[self.index] = self.data_scale * real_time_data[self.real_time_index]
            ...
            # print(f"Plot index: {self.index}, Real time index: {self.real_time_index}")
            # # Placeholder
            # pass
        ...
        self.after(4, self.update_plot, i + 1)
...
class PlotManager:
    ...
    def draw_line(self):
        ...
        self.plotter.create_line(self.plotter.index - 1,
                                 self.plotter.winfo_height() // 2 - self.plotter.plot_list[self.plotter.index - 1],
                                 self.plotter.index,
                                 self.plotter.winfo_height() // 2 - self.plotter.plot_list[self.plotter.index],
                                 width=3,
                                 fill="red",
                                 tags=self.tag)
    ...
    def draw_count(self):
        ...
        peaks = detectors.pan_tompkins_detector(self.plotter.plot_list)
        # print(peaks)
        print(len(peaks))
        self.plotter.create_text(100,490,
                                 text=f"Counted number of beats: {len(peaks)}",
    ...
    def draw_text(self):
        ...
        for i in range(self.plotter.horizontal_scale, self.plotter.winfo_width() + 1, self.plotter.horizontal_scale):
            self.plotter.create_text(i, 245, text=f"{i}", tags="texts")

        # Draw vertical grid lines
        for i in range(-250, 250, self.plotter.vertical_scale):
            self.plotter.create_text(15, i + 245, text=f"{-i}", tags="texts")
